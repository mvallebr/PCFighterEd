...
var
  i, j: integer;
begin
  grdInputs.Cells[1, 0] := 'ESQ';
  grdInputs.Cells[2, 0] := 'ACI';
  grdInputs.Cells[3, 0] := 'DIR';
  grdInputs.Cells[4, 0] := 'ABA';
...
begin
  traj := getCurrentTrajectory;
  if traj=nil then exit;
  traj.trigger := trim(edtTrigger.Text);
end;
...
begin
  ...
  try
    ...
    for i := 0 to lstActions.Items.Count -1 do begin
      ...
      naction := nactions.AddChild(action.caption);
      ...
      for j := 0 to action.frames.Count-1 do begin
        ...
        nframe:= naction.AddChild(frame.caption);
        ...
        for k := 0 to frame.polys.Count-1 do begin
          ...
          npoly := nframe.AddChild(poly.caption);
    ...
    for i := 0 to lstTrajectories.Items.Count -1 do begin
      ...
      ntraj := ntrajectories.AddChild(traj.caption);
      ...
      for j := 0 to traj.points.Count-1 do begin
        ...
        npoint:= ntraj.AddChild(point.caption);
    ...
    for i := 0 to lstTriggers.Items.Count -1 do begin
      ...
      ntrigger := ntriggers.AddChild(trigger.caption);
      ntrigger.Text := trigger.text;
    end;
    for i := 0 to lstStates.Items.Count -1 do begin
      nstates.AddChild(lstStates.Items.Strings[i]);
    end;
    for i := 0 to lstInputs.Items.Count -1 do begin
      input := TInput(lstInputs.Items.Objects[i]);
      ninput := ninputs.AddChild(input.caption);
      //props: array[1..10,1..16] of string;
      for j := 1 to 16 do begin
        ninputset:= ninput.AddChild('inputset'+IntToStr(j));
        ninputset.Attributes['ESC'] := input.props[1, j];
        ninputset.Attributes['ACI'] := input.props[2, j];
        ninputset.Attributes['DIR'] := input.props[3, j];
        ninputset.Attributes['ABA'] := input.props[4, j];
...
begin
  ...
  try
    ...
    for i := 0 to nactions.ChildNodes.Count -1 do begin
      ...
      action := TAction.Create(naction.NodeName);
      ...
      for j := 0 to naction.ChildNodes.Count -1 do begin
        ...
        frame := TFrame.Create(nframe.NodeName);
        action.frames.Add(frame);
        ...
        for k := 0 to nframe.ChildNodes.Count -1 do begin
          ...
          poly := TPoly.Create(npoly.NodeName);
          frame.polys.Add(poly);
    ...
    for i := 0 to ntrajectories.ChildNodes.Count -1 do begin
      ...
      traj := TTrajectory.Create(ntraj.NodeName);
      ...
      for j := 0 to ntraj.ChildNodes.Count -1 do begin
        ...
        point := TTrajPoint.Create(npoint.NodeName);
    ...
    for i := 0 to ntriggers.ChildNodes.Count -1 do begin
      ...
      trigger := TTrigger.Create(ntrigger.NodeName);
    ...
    for i := 0 to nstates.ChildNodes.Count -1 do begin
      lstStates.Items.Add(nstates.ChildNodes.Get(i).NodeName);
    end;

    for i := 0 to ninputs.ChildNodes.Count -1 do begin
      ninput := ninputs.ChildNodes.Get(i);
      input := TInput.Create(ninput.NodeName);
      ...
      for j := 1 to ninput.ChildNodes.Count do begin
        ...
        if j>16 then break;
        input.props[1, j] := ninputset.Attributes['ESC'];
        input.props[2, j] := ninputset.Attributes['ACI'];
        input.props[3, j] := ninputset.Attributes['DIR'];
        input.props[4, j] := ninputset.Attributes['ABA'];
